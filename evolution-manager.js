...
class EvolutionManager {
  ...
  constructor() {
    this.groupIdsToNotScore = ['leftover'];
  }
  ...
  getScoreForGroup(group) {
    if (group.length === 0 || _.includes(this.groupIdsToNotScore, group.id)) {
      return 0;
    }
  ...
  getScoreForCommunity(community) {
    if (community && 'groups' in community) {
  ...
  fitness(communityJsonObject) {
    let community = this.getCommunityFromJsonObject(communityJsonObject);
    return this.getScoreForCommunity(community);
  }
}
